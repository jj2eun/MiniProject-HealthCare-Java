...
//            System.out.println(Eat_date);
            ...
            try {
               ...
               ResultSet rs_morning = db.getInfo("select nutrient.Food_Name, user_eat.Food_cal, user_eat.Food_Count from user_eat left outer join nutrient on user_eat.Food_no = nutrient.Food_no where Eat_Time='아침' and Eat_date=" + "'" + selectedDate + "'");
               ResultSet rs_lunch = db.getInfo("select nutrient.Food_Name, user_eat.Food_cal, user_eat.Food_Count from user_eat left outer join nutrient on user_eat.Food_no = nutrient.Food_no where Eat_Time='점심' and Eat_date=" + "'" + selectedDate + "'");
               ResultSet rs_dinner = db.getInfo("select nutrient.Food_Name, user_eat.Food_cal, user_eat.Food_Count from user_eat left outer join nutrient on user_eat.Food_no = nutrient.Food_no where Eat_Time='저녁' and Eat_date=" + "'" + selectedDate + "'");
               ResultSet rs_dessert = db.getInfo("select nutrient.Food_Name, user_eat.Food_cal, user_eat.Food_Count from user_eat left outer join nutrient on user_eat.Food_no = nutrient.Food_no where Eat_Time='간식' and Eat_date=" + "'" + selectedDate + "'");
...
//               if (rs_morning != null) try { rs_morning.close(); } catch(SQLException ex) {}
//               if (rs_lunch != null) try { rs_lunch.close(); } catch(SQLException ex) {}
//               if (rs_dinner != null) try { rs_dinner.close(); } catch(SQLException ex) {}
//               if (rs_dessert != null) try { rs_dessert.close(); } catch(SQLException ex) {}
